...
namespace ZUMOAPPNAMEService
{
    ...
    public class ZUMOAPPNAMEInitializer : CreateDatabaseIfNotExists<ZUMOAPPNAMEContext>
    {
        ...
        {
            List<Note> todoItems = new List<Note>
            {
                new Note { Id = Guid.NewGuid().ToString(), Text = "First item", Complete = false },
                new Note { Id = Guid.NewGuid().ToString(), Text = "Second item", Complete = false },
            };

            foreach (Note todoItem in todoItems)
            {
                context.Set<Note>().Add(todoItem);
            }
